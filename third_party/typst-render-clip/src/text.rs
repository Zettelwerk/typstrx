...
pub fn render_text(canvas: &mut sk::Pixmap, state: State, text: &TextItem) {
    ...
    for glyph in &text.glyphs {
        ...
        let placed = state.pre_translate(Point::new(x_offset, -y_offset));

        if !glyph_may_touch_canvas(canvas, &placed.transform, &text.font, id, text.size) {
            // Off-canvas: skip before either branch does its expensive work.
        ...
        if should_outline(&text.font, id) {
            render_outline_glyph(canvas, placed, text, id);
        } else {
            let upem = text.font.units_per_em();
            let text_scale = text.size / upem;
            let state = placed.pre_scale(Axes::new(text_scale, text_scale));
...
/// Whether a glyph placed at `ts` can put any ink on the canvas.
///
/// ADDED (not in upstream typst-render). With whole-page rendering this would
/// always be true and the check would be waste; with [`crate::render_tile`] the
/// canvas is a small window of the page, so most of a page's glyphs fall
/// entirely outside it. Both glyph paths do real work before their output is
/// clamped to the canvas — `rasterize` builds a coverage bitmap, `glyph_frame`
/// decodes a colour glyph — so the rejection has to happen ahead of both.
///
/// Conservative by construction: it errs towards keeping a glyph, and a font
/// with no bounding box for the glyph is never skipped.
fn glyph_may_touch_canvas(
    canvas: &sk::Pixmap,
    ts: &sk::Transform,
    font: &FontInstance,
    id: GlyphId,
    ...
    let Some(bbox) = font.ttf().glyph_bounding_box(id) else {
        // No bounding box in the font: the glyph could be anything, including
        // the tall variants maths uses (integrals, radicals, brackets), which
        // reach well past the em square. Keep it.
        return true;
    };

    // Font units -> text-space points. Design boxes are Y-up, the render
    // transform is Y-down, so the box flips.
    let s = size.to_f32() / font.units_per_em() as f32;
    let (x0, y0) = (bbox.x_min as f32 * s, -(bbox.y_max as f32) * s);
    let (x1, y1) = (bbox.x_max as f32 * s, -(bbox.y_min as f32) * s);

    // Map all four corners rather than just two, so skew and anisotropic
    // scaling widen the box instead of letting the glyph escape it.
    let mut corners = [
        sk::Point::from_xy(x0, y0),
        sk::Point::from_xy(x1, y0),
        sk::Point::from_xy(x0, y1),
        sk::Point::from_xy(x1, y1),
    ];
    ...
    // A pixel of slack so antialiasing along the canvas edge is never clipped.
    max_x >= -1.0
        && max_y >= -1.0
        && min_x <= canvas.width() as f32 + 1.0
        && min_y <= canvas.height() as f32 + 1.0
}
